...
<script type="text/javascript">
	...
	function login(){
		...
		if(username!=""&&username!="用户名/邮箱/手机号"&&password!=""&&password!="密码"){
			$("#logform").ajaxSubmit({
		   	     type: "post",
		   	     url: "${pageContext.request.contextPath}/index_login.action",
		   	     dataType: "json",
	...
	function checkUsername(){
		...
		if(username!=""&&username!="用户名"){
			$.post("${pageContext.request.contextPath}/index_checkUsername.action",'username='+username,function(data){
				if(data.flag){
	...
	function checkEmail(){
		...
		var reg=/^(\w)+(\.\w+)*@(\w)+((\.\w{2,5}){1,3})$/;
       	if(reg.test(email)){
       		$.post("${pageContext.request.contextPath}/index_checkEmail.action",'email='+email,function(data){
				if(data.flag){
	...
	function regist(){
		if(checkUsername()&&checkEmail()&&checkPassword()&&checkRepassword()&&checkCode()){
			$("#registform").ajaxSubmit({
		   	     type: "post",
		   	     url: "${pageContext.request.contextPath}/index_regist.action",
		   	     dataType: "json",
		   	     success: function(data){
		   	           //返回提示信息       
		   	    	 if(data.flag){
	...
	function changimg(){
		...
		$('#recheckcode').val('');
		img1.src="${pageContext.request.contextPath}/index_checkImg.action?"+new Date().getTime();
	}
	...
	function checkPhone(){
		...
		//alert(phone);
		if(phone!=""&&phone!="手机号"){
			$.get("${pageContext.request.contextPath}/index_checkPhone.action",'phone='+phone,function(data){
				if(data.flag){
	...
	function sendSms(){
		...
		if(phone!=""&&phone!="手机号"){
			if(checkPhone()){
				$.post("${pageContext.request.contextPath}/index_sendSms.action",'phone='+phone,function(data){
					if(data.flag){
	...
	function phoneRegist(){
		if(checkPhone()&&checkPhoneUsername()&&checkPhonePassword()&&checkPhoneRepassword()&&checkPhoneCode()){
			$("#phoneregistform").ajaxSubmit({
		   	     type: "post",
		   	     url: "${pageContext.request.contextPath}/index_phoneRegist.action",
		   	     dataType: "json",
		   	     success: function(data){
		   	           //返回提示信息       
		   	    	 if(data.flag){
	...
	function forgetPassword(){
		window.location.href="${pageContext.request.contextPath}/userno_forgetPasswordPage.html";
	}
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                    	<div class="row">
                                    		...
                                    		<div class="col-sm-3">
                                    		<img id="checkimg"  src="${pageContext.request.contextPath}/index_checkImg.action"  onclick="changimg()" title="点击更换验证码">
                                    		</div>
